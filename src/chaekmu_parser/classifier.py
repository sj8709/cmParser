...
# 필드 라벨 정규식
# ---------------------------------------------------------------------------
_FN = r"\s*\d*\s*\)?\s*"  # 각주 번호 접미사 (예: '직책1)', '책무 개요1)')

LABEL_POSITION      = re.compile(rf"^\s*직\s*책{_FN}$")
LABEL_NAME          = re.compile(rf"^\s*성\s*명{_FN}$")
LABEL_TITLE         = re.compile(rf"^\s*직\s*위{_FN}$")
LABEL_APPT_DATE     = re.compile(rf"^\s*현\s*직\s*책\s*부\s*여\s*일{_FN}$")
LABEL_CONCUR_YN     = re.compile(rf"^\s*겸\s*직\s*(?:여\s*부)?{_FN}$")
LABEL_CONCUR_DTL    = re.compile(rf"^\s*겸\s*직\s*사\s*항{_FN}$")
LABEL_DEPT          = re.compile(rf"^\s*소\s*관\s*부\s*서{_FN}$")
LABEL_COMMITTEE_ROOT = re.compile(rf"^\s*주\s*관\s*회\s*의\s*체{_FN}$")
LABEL_ASSIGN_DATE   = re.compile(rf"^\s*책\s*무\s*배\s*분\s*일\s*자{_FN}$")
LABEL_RESP_SUMMARY  = re.compile(rf"^\s*책\s*무\s*개\s*요{_FN}$")
